...
## Recent Changes
- 2026-03-16: iOS build `19f13598` succeeded (v1.5.0, build 28) and submitted to App Store Connect. Widget extension deferred — EAS Build SDK 52 can't auto-provision extension targets (requires separate App ID + provisioning profile on Apple Developer Portal). Widget code preserved in `faith-empire-app/widget-extension/` and `faith-empire-app/plugins/withWidget.js` for future SDK 53+ integration via `@bacons/apple-targets`. Config plugin disabled in `app.config.js`.
- 2026-03-15: Added Listen Along (text-to-speech) feature using browser SpeechSynthesis API. Bible reader has play/pause/stop controls with speed adjustment (0.75x/1x/1.35x). Decoded books have additional mode toggle to listen to either the decoded version or original KJV. Active verse highlights as it's read aloud. Voice picker lets users choose from device voices (persisted in localStorage). Continuous play ("Auto") mode auto-advances through chapters so users can listen to entire books start to finish. Files: `client/src/lib/use-speech.ts` (hook), `client/src/components/speech-controls.tsx` (UI controls).
